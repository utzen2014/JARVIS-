    ...
    def _build_social_bar(self):
        ...
        def _load_icon(filename: str):
            try:
                ...
                print(f"Logo Yükleme Hatası: {e}")  # Bir hata olursa terminalde görebilmek için
                return None

      # 1. "Produced by" Yazısı
        name_lbl = tk.Label(
            bar, text="Produced by",
            fg="#3a8a82", bg=C_BG,
            font=font_display(14),
            justify="left",
        )
        # pady=(0, 15) -> Üstte 0, Altta 15 piksel boşluk bırakır (yazıyı yukarı iter)
        name_lbl.pack(side="left", padx=(0, 6), pady=(0, 15)) 

        # 2. Icon Klasöründeki Logonu Yükleme
        self._my_logo = _load_icon("UZ logo.png")

        # 3. Logoyu Ekrana Basma
        if self._my_logo:
            logo_lbl = tk.Label(bar, image=self._my_logo, bg=C_BG)
            # Logoyu da yazıyla aynı hizada yukarı kaldırmak için dikey boşluk veriyoruz
            logo_lbl.pack(side="left", padx=2, pady=(0, 15))
